...
module prif_init_test_m
  ...
  type, extends(prif_test_t) :: prif_init_test_t
  contains
...
contains
  ...
  function results() result(test_results)
...
#else
    procedure(diagnosis_function_i), pointer :: check_caffeination_ptr, check_redundant_prif_init_call_ptr 
    ...
    function check_redundant_prif_init_call()  result(test_diagnosis)
      type(test_diagnosis_t) test_diagnosis
      integer stat
      call prif_init(stat)
      call prif_init(stat)
      test_diagnosis = test_diagnosis_t( &
         test_passed = stat == PRIF_STAT_ALREADY_INIT &
        ,diagnostics_string = "expected: " //string_t(PRIF_STAT_ALREADY_INIT) // "(PRIF_STAT_ALREADY_INIT), actual: " // string_t(stat) &
      )
    end function
