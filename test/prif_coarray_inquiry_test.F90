...
contains
  ...
  impure elemental function check_cobound(corank, omit_trailing) result(diag)
    ...
    if (omit_trailing) then
      ...
      call prif_allocate_coarray( lcobounds, leading_ucobounds, data_size, null_final_func, &
        coarray_handle, allocated_memory)
    else
      call prif_allocate_coarray( lcobounds, ucobounds, data_size, null_final_func, &
        coarray_handle, allocated_memory)
    end if
