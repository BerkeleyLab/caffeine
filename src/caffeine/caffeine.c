...
void c_co_reduce_universal(void* c_loc_a, size_t Nelem, int* stat, int* result_image, int dt, size_t sizeof_type, int op)
{
     gex_Event_t ev;

     if (result_image == NULL) {
       ev = gex_Coll_ReduceToAllNB(myteam, c_loc_a, c_loc_a, dt, sizeof_type, Nelem, op, NULL, NULL, 0);
     } else {
       ev = gex_Coll_ReduceToOneNB(myteam, (*result_image)-1, c_loc_a, c_loc_a, dt, sizeof_type, Nelem, op, NULL, NULL, 0);
     }
     gex_Event_Wait(ev);  
